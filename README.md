...
This repo is continuation of all work done by @Kulbear for this specialization. All the code base, quiz questions, screenshot, and images, are taken from, unless specified, [Deep Learning Specialization on Coursera](https://www.coursera.org/specializations/deep-learning).  Focus here are making sure dependencies are all accounted for.  
...
```
2. Create virtual environment using python3 venv
```bash
...
```
4. Now run the kernel "self-install" script:
```bash
...
```
6. You should now be able to see your kernel in the IPython notebook menu: `Kernel -> Change kernel`. 
